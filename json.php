...
{ 
  ...
  foreach ($template as $col)
  {
    ...
    {
      case "Name":
        $row_vals['Name'] = generate_name($col[2][0], $g_male_names, $g_female_names, $g_names, $g_surnames);
        break;

      case "Phone":
        $row_vals['Phone'] = generate_random_num_str($col[2][0]);
        break;

      case "Email":
        $row_vals['Email'] = generate_email_address($g_words);
        break;
        
      case "Street-Address":
        $row_vals['Street-Address'] = generate_street_address($g_words);
        break;

      case "City":
        $row_vals['City'] = $g_cities[rand(0, count($g_cities)-1)];
        break;
      ...
      case "Postal-Zip":
        ...
        $options_to_generate = array();
				...
				if (!empty($options_to_generate))
				{
				  ...
				  $row_vals['Postal-Zip'] = generate_random_alphanumeric_str($random_zip_format);
				}
        break;
      ...
      case "State-Province":
        ...
        $row_vals['State-Province'] = $places[rand(0, count($places)-1)];
        break;
        
      case "Country":
        $row_vals['Country'] = $g_countries[rand(0, count($g_countries)-1)];
        break;
      ...
      case "Date":
        ...
        $row_vals['Date'] = date($col[2][0], $rand_date);
        break;
        
      case "Text-Fixed":
        $row_vals['Text-Fixed'] = generate_random_text_str($g_words, false, "fixed", $col[2][0]);
        break;
